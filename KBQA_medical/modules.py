...
from NLU4.sklearn_Classification.clf_model import CLFModel
...
clf_model = CLFModel('./NLU4/sklearn_Classification/model_file/')

def intent_classifier(text):
    url = 'http://localhost:60062/service/api/bert_intent_recognize'
    data = {"text":text}
...
def slot_recognizer(text):
    url = 'http://localhost:60061/service/api/medical_ner'
    data = {"text_list":[text]}
    headers = {'Content-Type':'application/json;charset=utf8'}
    reponse = requests.post(url,data=json.dumps(data),headers=headers)
    if reponse.status_code == 200:
