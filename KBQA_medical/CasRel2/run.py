...
parser.add_argument('--dataset_dir', default='./data/baiduie_train_data', type=str, help='')
args = parser.parse_args()

if __name__ == '__main__':
    # pre-trained bert model config
    bert_model = './bert_weight_files'
    ...
    if args.train:
        ...
        precision, recall, f1_score = metric(subject_model, object_model, test_data, id2rel, tokenizer, isExactMatch,
                                             test_result_path)
        print(f'precision:{0}\trecall:{1}\tf1_score:{2}'.format(round(precision, 4), round(recall, 4),
                                                                round(f1_score, 4)))