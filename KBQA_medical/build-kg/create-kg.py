...
def print_data_info(data_path):  # 打印数据信息
    triples = []
    i = 0
...
class MedicalExtractor(object):
    ...
    def extract_triples(self, data_path):
        ...
        with open(data_path, 'r', encoding='utf-8') as f:
            for line in tqdm(f.readlines(), ncols=80):
                ...
                # print(type(data_json))  # 字典dict
                disease_dict = {}  # {'name':'糖尿病'} key = name
                disease = data_json['name']
                disease_dict['name'] = disease
                self.diseases.append(disease)  # 所有疾病节点
                ...
                if 'symptom' in data_json:  # 疾病的三元组（对象属性）
                    self.symptoms += data_json['symptom']  # 所有症状节点的列表
                    # print(self.symptoms)
                    for symptom in data_json['symptom']:  # 判断某疾病是否有某症状
                        # print(symptom)
                ...
                if 'desc' in data_json:  # 疾病的属性（数据属性）
                ...
                if 'cure_department' in data_json:  # 对象属性
    ...
    def export_entitys_relations(self):
        self.export_data(self.drugs, './data/drugs.json')
        self.export_data(self.recipes, './data/recipes.json')
        self.export_data(self.foods, './data/foods.json')
        self.export_data(self.checks, './data/checks.json')
        self.export_data(self.departments, './data/departments.json')
        self.export_data(self.producers, './data/producers.json')
        self.export_data(self.diseases, './data/diseases.json')
        self.export_data(self.symptoms, './data/symptoms.json')

        self.export_data(self.rels_department, './data/rels_department.json')
        self.export_data(self.rels_noteat, './data/rels_noteat.json')
        self.export_data(self.rels_doeat, './data/rels_doeat.json')
        self.export_data(self.rels_recommandeat, './data/rels_recommandeat.json')
        self.export_data(self.rels_commonddrug, './data/rels_commonddrug.json')
        self.export_data(self.rels_recommanddrug, './data/rels_recommanddrug.json')
        self.export_data(self.rels_check, './data/rels_check.json')
        self.export_data(self.rels_drug_producer, './data/rels_drug_producer.json')
        self.export_data(self.rels_symptom, './data/rels_symptom.json')
        self.export_data(self.rels_acompany, './data/rels_acompany.json')
        self.export_data(self.rels_category, './data/rels_category.json')
